Shader "SkySoft/UIBlur"
{
    ...
    [PerRendererData] _MainTexture("Sprite Texture", 2D) = "white" {}
    _Color("Tint", Color) = (1, 1, 1, 1)
        ...
        _ColourMask("Colour Mask", Float) = 15
...
#pragma multi_compile_local __ UNITY_UI_ALPHACLIP
        ...
        struct appdata_t 
        {
            ...
            half4 GetBlur(float uv, half4 pixel) 
            {
#define GrabPixelXY(kernelx, kernely) tex2Dproj(_BlurTex), UNITY_PROJ_COORD(float4(uv.x + _Jump * kernelx, uv.y + _Jump * kernely, uv.z, uv.w));
                ...
                for (; range <= _Radius; range += _Step) 
                {
                    for (float i = 0.06; i <= 0.18; i += 0.03) 
                    {
                        ...
                        sum += GrabPixelXY(range * i, -range * i);
